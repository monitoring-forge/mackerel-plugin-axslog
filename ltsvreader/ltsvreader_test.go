...
func TestParseMultiStatus(t *testing.T) {
	r := New("reqtime", []string{"status", "code"})
	i, rt, st := r.Parse([]byte("time:08/Mar/2017:14:12:40 +0900	code:404	status:200	reqtime:0.030	host:10.20.30.40	req:GET /example/path HTTP/1.1	method:GET	size:941	ua:Mozilla/5.0 (Linux; Android 4.4.2; SO-01F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/73.0.3683.90 Mobile Safari/537.36"))
	if i != axslog.AllFlagOK {
		t.Error("allflag not ok")
	}
	if !bytes.Equal(rt, []byte("0.030")) {
